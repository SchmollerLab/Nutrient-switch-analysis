...
def get_analysed_remote_path(file_path, data_root, raw_file_names):
    """Return the raw location when a file is also included in the raw manifest."""
    if file_path.name.casefold() in raw_file_names:
        return get_remote_path("raw", file_path.name)
    return get_remote_path("analysed", file_path.relative_to(data_root))
    ...
    for experiment_folder, annotation in file_annotation_dict.items():
        ...
        for position_path in sorted(position_root.glob("Position_*")):
            ...
            for file_path in image_files:
                ...
                rows.append(
                    {
                        "Files": get_analysed_remote_path(
                            file_path, data_root, raw_file_names
                        ),
                        "source_image": get_analysed_remote_path(
                            source_path, data_root, raw_file_names
                        ),
        ...
        for file_path in sorted(experiment_path.rglob("*")):
            ...
            if (
                ...
                or file_path.name.casefold() in raw_file_names
                or any(
